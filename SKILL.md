...
| App pricing | `storescreens pricing ...` | `pricing_*` | App price schedule: `get` current per-territory prices, `set` (free / single paid tier / per-territory overrides - amounts are local-currency and snap to the nearest valid tier), and `price-points` discovery (list a territory's tiers or find the nearest to an amount). Same set-on-submit path is the `pricing:` block in `storescreens.yml`. |
| App Review submissions | `storescreens review-submissions ...` | `review_submissions_*` | Direct control over the `reviewSubmissions` flow that `submit --submit-for-review` drives automatically: list / get / items (with per-item review states), create draft, add-item (appStoreVersion shorthand plus CPP versions, app events, experiments, Background Asset + Game Center versions), submit draft, cancel an in-flight submission (`PATCH canceled:true` - the queued / in-review / rejected withdraw path), item update (removed / resolved) + delete. Plus `app_store_versions_list` (MCP) for finding version ids. |
...
To act on what `status` shows, use `storescreens review-submissions` (MCP: `review_submissions_*`): `cancel <id>` withdraws a queued / in-review / rejected submission (freeing the version for a fixed resubmit), `items <id>` shows per-item review states (`REJECTED` pinpoints what Apple objected to), and `create` / `add-item` / `submit` assemble a submission by hand - including non-version items like Custom Product Page versions or app events.
